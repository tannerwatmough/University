...
/*
Assignment 4: Splash Screen Styles JavaScript
Author: Tanner Wamtough
...
/* 
  Birthday Array
  TypeError if Array was not before the code
*/
...
// New Date() object is created with user's computer date assigned to variable
...
/* Calls the showDate() which updates page with user's current date */
showDate();

/* Calls the dayNumber() which provides day total (dTotal) parameter for showBirthday(); */
dayNumber();

/*
  Calls the showBirthday() which updates page with correpsonding famous
  birthday 
*/
showBirthday(day);
...
function showDate() {
  // Parses currentDate for the numeric year
  var cYear = currentDate.getFullYear();
  // Array to convert month 0-11 to corresponding month name. 
  var months = ["January", "February", "March", "April", "May", "June", 
    "July", "August", "September", "October", "November", "December"];
  // Grabs month from current date as 0-11 value which corresponds to array
  var cMonth = months[currentDate.getMonth()];
  // Array to convert day 0-6 to corresponding day name. 
  var dayName = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", 
    "Friday", "Saturday", "Sunday"];
  // Grabs day from current date as 0-6 value corresponding to array
  var cDay = dayName[currentDate.getDay()];
  // Returns day number
  var cDate = currentDate.getDate();

  // Gets span with id "currentDate" and modifies its innertext
  document.getElementById("currentDate").innerText = cDay + ", " + cMonth +
   " " + cDate + ", " + cYear;  
}

function dayNumber() {
  // Day number within a month (1-31)
  var dNumber = currentDate.getDate();

  // Determines the current month (0-11)
  switch (currentDate.getMonth()) {
    case 0: // January
      day = dNumber;
      return day;
    case 1: // February 
      day = dNumber + 31; // Adding previous month's days to current date. 
      return day;
    case 2: // March
      day = dNumber + 60; // Adding all previous month's days to current date. 
      /* Interestingly, even with leap years, it should always be 60 since the
      leap day is always included in the array list. */
      return day;
    case 3: // April
      day = dNumber + 91; // Adding all previous month's days to current date. 
      return day;
    case 4: // May
      day = dNumber + 121; // Adding all previous month's days to current date. 
      return day;
    case 5: // June
      day = dNumber + 152; // Adding all previous month's days to current date. 
      return day;
    case 6: // July
      day = dNumber + 182; // Adding all previous month's days to current date. 
      return day; 
    case 7: // August
      day = dNumber + 213; // Adding all previous month's days to current date. 
      return day;
    case 8: // September
      day = dNumber + 244; // Adding all previous month's days to current date. 
      return day;
    case 9: // October
      day = dNumber + 274; // Adding all previous month's days to current date. 
      return day;
    case 10: // Novemeber
      day = dNumber + 305; // Adding all previous month's days to current date. 
      return day;
    case 11: // December 
      day = dNumber + 335; 
      /* 
        Adding all prev month's days to current. Combo of Dec 31 and 335 will be 366 which corresponds to DEc 31 in the array due to including the leap day in normal count.
      */
      return day; // Returns total from function which is passed into births[] in showBirthday()
  }
}

function showBirthday(day) {
  // Matches births array to day returned by dayNumber() funciton
  var cBirthday = births[day];
  // Gets span with id "famousBirthday" and modifies its inner text
  document.getElementById("famousBirthday").innerText = cBirthday;
}