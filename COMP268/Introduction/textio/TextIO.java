 ...
 * default, it reads
 * from standard input and writes to standard output, but it is possible to
 * redirect the input
 ...
 * the user is
 * repeatedly prompted for input until a legal input is entered. (If standard
 * input has been
 * changed externally, as by file redirection on the command line, this is not a
 * reasonable
 * behavior; to handle this case, TextIO will give up after 10 consecutive
 * illegal inputs and
 ...
 * usable even by a beginning programmer who has not yet learned about objects
 * and exceptions. Therefore,
 * everything is in a single source file that compiles into a single class file,
 * all the methods are
 * static methods, and none of the methods throw exceptions that would require
 * try...catch statements.
 * Also for this reason, all exceptions are converted into
 * IllegalArgumentExceptions, even when this
 * exception type doesn't really make sense.
 * <p>
 * This class requires Java 5.0 or higher. (A previous version of TextIO
 * required only Java 1.1;
 ...
 * <p>
 * TextIO must be imported from package textio to be used. (Previous versions
 * were in the
 * default package. Alternatively, this class can be moved into the default
 * package by deleting
...
public class TextIO {
    ...
    /*
     * Modified May 2018 to move TextIO.java into package textio. This class is
     * otherwise
     * identical to the version in the default package.
     */

    /*
     * Modified November 2007 to empty the TextIO input buffer when switching from
     * one
     * input source to another. This fixes a bug that allows input from the previous
     * input
     ...
     * is in the default state). If a file or stream was previously the input
     * source, that file
     * or stream is closed.
     */
    ...
    /**
     * Opens a file with a specified name for input. If the file name is null, this
     * has
     * the same effect as calling readStandardInput(); that is, input will be read
     * from standard
     * input. If an
     * error occurs while trying to open the file, an exception of type
     ...
     * select
     * an input file. If the user cancels the dialog instead of selecting a file, it
     * is
     * not considered an error, but the return value of the subroutine is false.
     * If the user does select a file, but there is an error while trying to open
     * the
     * file, then an exception of type IllegalArgumentException is thrown. Finally,
     * if
     * the user selects a file and it is successfully opened, then the return value
     * of the
     * subroutine is true, and the input routines will read from the file, instead
     * of
     ...
     * continue
     * to run even if the thread that runs the main program ends. If you use this
     * method
     * in a non-GUI program, it might be necessary to call System.exit(0) at the end
     * of the main()
     ...
     * is in the default state). If a file or stream was previously open for output,
     * it
     * will be closed.
     */
    ...
    /**
     * Opens a file with a specified name for output. If the file name is null, this
     * has
     ...
     * called,
     * all of the output routines will write to the file, instead of to standard
     * output.
     ...
     * Puts a GUI file-selection dialog box on the screen in which the user can
     * select
     * an output file. If the user cancels the dialog instead of selecting a file,
     * it is
     * not considered an error, but the return value of the subroutine is false.
     * If the user does select a file, but there is an error while trying to open
     * the
     ...
     * the user selects a file and it is successfully opened, then the return value
     * of the
     * subroutine is true, and the output routines will write to the file, instead
     * of
    ...
    /**
     * If TextIO is currently reading from a file, then the return value is the name
     * of the file.
     * If the class is reading from standard input or from a stream, then the return
     * value is null.
     */
    ...
    /**
     * If TextIO is currently writing to a file, then the return value is the name
     * of the file.
     * If the class is writing to standard output or to a stream, then the return
     * value is null.
     */
     ...
     * Write a single value to the current output destination, using the default
     * format
     * and outputting at least minChars characters (with extra spaces added before
     * the
     * output value if necessary). This method will handle any type of parameter,
     * even one
     * whose type is one of the primitive types.
     * 
     * @param x        The value to be output, which can be of any type.
     * @param minChars The minimum number of characters to use for the output. If x
     *                 requires fewer
     *                 then this number of characters, then extra spaces are added
     *                 to the front of x to bring
     *                 the total up to minChars. If minChars is less than or equal
     *                 to zero, then x will be printed
    ...
    /**
     * Writes formatted output values to the current output destination. This method
     * has the
     * same function as System.out.printf(); the details of formatted output are not
     * discussed
     * here. The first parameter is a string that describes the format of the
     * output. There
     * can be any number of additional parameters; these specify the values to be
     * output and
     * can be of any type. This method will throw an IllegalArgumentException if the
     * format string is null or if the format string is illegal for the values that
     * are being
     * output.
     */
     ...
     * this method does NOT skip whitespace before testing for end-of-line -- if you
     * want to do
    ...
    /**
     * Reads the next character from the current input source. The character can be
     * a whitespace
     * character; compare this to the getChar() method, which skips over whitespace
     * and returns the
     * next non-whitespace character. An end-of-line is always returned as the
     * character '\n', even
     * when the actual end-of-line in the input source is something else, such as
     * '\r' or "\r\n".
     * This method will throw an IllegalArgumentException if the input is at
     * end-of-file (which will
     ...
     * removing that
     * character from the input. The character can be a whitespace character and can
     * be the
     * end-of-file character (specified by the constant TextIO.EOF).An end-of-line
     * is always returned
     * as the character '\n', even when the actual end-of-line in the input source
     * is something else,
     ...
     * method is called,
     * the next input character is either an end-of-line, an end-of-file, or a
     * non-whitespace character.
     * This method never causes an error. (Ordinarily, end-of-file is not possible
     * when reading from
     * standard input.)
     */
    ...
    /**
     * Skips over any whitespace characters, including for end-of-lines. After this
     * method is called,
     ...
     * is input. In other cases, an IllegalArgumentException will be thrown if a
     ...
     * the current line of input (including the next end-of-line character, if any).
     * Note that the value
     * that is returned will be a non-whitespace character; compare this with the
     * getAnyChar() method.
     * When using standard IO, this will not produce an error. In other cases, an
     * error can occur if
     ...
     * legal value is not found.
     * <p>
     * Legal inputs for a boolean input are: true, t, yes, y, 1, false, f, no, n,
     * and 0; letters can be
     * either upper case or lower case. One "word" of input is read, using the
     * getWord() method, and it
     * must be one of these; note that the "word" must be terminated by a whitespace
    ...
    /**
     * Skips whitespace characters and then reads one "word" from input, discarding
     * the rest of
     * the current line of input (including the next end-of-line character, if any).
     * A word is defined as
     * a sequence of non-whitespace characters (not just letters!). When using
     * standard IO,
     * this will not produce an error. In other cases, an IllegalArgumentException
     * will be thrown
     ...
     * end-of-line. The end-of-line
     * is read but is not included in the return value. Any other whitespace
     * characters on the line are retained,
     * even if they occur at the start of input. The return value will be an empty
     * string if there are
     * no characters before the end-of-line. When using standard IO, this will not
     * produce an error.
     * In other cases, an IllegalArgumentException will be thrown if an end-of-file
     * is encountered.
     */
     ...
     * the current line of input are retained, and will be read by the next input
     * operation. When using standard IO,
     * this will not produce an error; the user will be prompted repeatedly for
     * input until a legal value
     * is input. In other cases, an IllegalArgumentException will be thrown if a
     * legal value is not found.
     ...
     * from input. Any additional characters on
     * the current line of input are retained, and will be read by the next input
     * operation. When using standard IO,
     * this will not produce an error. In other cases, an IllegalArgumentException
     * will be thrown if an end-of-file
     * is encountered.
     */
     ...
     * the current line of input are retained, and will be read by the next input
     * operation. A word is defined as
     * a sequence of non-whitespace characters (not just letters!). When using
     * standard IO,
     * this will not produce an error. In other cases, an IllegalArgumentException
     * will be thrown
